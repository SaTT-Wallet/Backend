...
exports.emailConnection = async (req, res, next) => {
  passport.authenticate('signinEmailStrategy', { session: false }, (err, user, info) => {
    if (err) {
...
exports.linkFacebookAccount = async (req, accessToken, refreshToken, profile, cb) => {
  ...
  if (users.length) {
    ...
    await app.db
      ...
      .updateOne(
        { _id: user_id },
        { $set: { idOnSn: profile._json.token_for_business, email: profile._json.email } }
      );
    cb(null, profile, {
      status: true,
...
exports.linkGoogleAccount = async (req, accessToken, refreshToken, profile, done) => {
  ...
  if (userExist.length) {
    ...
    await app.db
      ...
      .updateOne(
        { _id: user_id },
        { $set: { idOnSn2: profile.id, email: profile.emails[0].value } }
      );
...
exports.addFacebookChannel = async (req, accessToken, refreshToken, profile, cb) => {
  ...
  if (fbProfile) {
    await app.db.fbProfile().updateOne({ UserId }, { $set: { accessToken: longToken } });
  } else {
    ...
    for (let i = 0; i < linkedinPages.elements.length; i++) {
      ...
      if (elem.state !== 'REVOKED') {
        ...
        elem.photo = linkedinPages.elements[i]['organization~'].logoV2
          ? linkedinPages.elements[i]['organization~'].logoV2['original~'].elements[0]
...
exports.addyoutubeChannel = async (req, accessToken, refreshToken, profile, cb) => {
  ...
  if (channelGoogle.length > 0) {
    ...
    var result = await rp({
      uri: 'https://www.googleapis.com/youtube/v3/channels',
      qs: { id: channelId, key: app.config.gdataApiKey, part: 'statistics,snippet' },
      json: true,
    });
    user_google = {};
