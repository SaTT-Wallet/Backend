module.exports = function (app) {
	...
	app.post('/campaign/abigen', async function(req, response) {
		...
		try {
			var res = await app.crm.auth( req.body.token);
			var cred = await app.account.unlock(res.id,pass);
			var ret = await app.campaign.abiGen();
			response.end(JSON.stringify(ret));
	...
	app.post('/oracle/abigen', async function(req, response) {
		...
		try {
			var res = await app.crm.auth( req.body.token);
			var cred = await app.account.unlock(res.id,pass);
			var ret = await app.oracleManager.abiGen();
			response.end(JSON.stringify(ret));
