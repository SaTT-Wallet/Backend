	 ...
	 passport.use(new emailStrategy(
		...
		if(users.length)
		{
			...
			if(res[0].password == synfonyHash(password))
			{
				...
				return done(null,{id:res[0].id,token:token,expires_in:date});
			}
			else
		...
		else
		{
			...
			var res_ins = await app.db.insert("INSERT INTO user SET ?",{username:username,email:username,password:synfonyHash(password),created:mydate,updated:mydate,confirmation_token:code,enabled:1});
			...
			return done(null,{id:id,token:token,expires_in:date});
