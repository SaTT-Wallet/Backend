module.exports = async function (app) {
  ...
  var ctrBonus = new app.web3.eth.Contract(
    app.config.ctrs.priceGap.abi,
    app.config.ctrs.priceGap.address.mainnet
  );
  const bad_login_limit = app.config.bad_login_limit;
  const { createLogger, format, transports } = require('winston');
  const { combine, timestamp, label, prettyPrint, myFormat } = format;

  var ctrwSaTT = new app.web3.eth.Contract(
    app.config.ctrs.wSaTT.abi,
    app.config.ctrs.wSaTT.address.mainnet
  );
  ...
  accountManager.recover = async function (userId, wordlist, oldpass, pass) {
    return new Promise(async (resolve, reject) => {
      ...
      var result = await app.db
        ...
        .updateOne(
          { UserId: parseInt(userId) },
          { $set: { keystore: account, num: count, btc: btcWallet, mnemo: mnemonic } }
        );
    });
  };
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (account.mnemo && !account.btc.addressSegWitCompat) {
          ...
          var result = await app.db
            .wallet()
            .updateOne({ UserId: parseInt(userId) }, { $set: { btc: btcWallet } });
        }
  ...
  accountManager.genBtcWallet = async function (pass) {
    ...
    var priv = child
      .execSync(
        app.config.bxCommand + ' seed -b 256 | ' + app.config.bxCommand + ' ec-new ',
        app.config.proc_opts
      )
      .toString()
      .replace('\n', '');
    var wif = child
      .execSync(app.config.bxCommand + ' ec-to-wif ' + priv, app.config.proc_opts)
      .toString()
      .replace('\n', '');
    var ek = child
      .execSync(
      ...
      .replace('\n', '');
    var pub = child
      .execSync(app.config.bxCommand + ' ec-to-public ' + priv, app.config.proc_opts)
      .toString()
      .replace('\n', '');
    ...
    return new Promise(async (resolve, reject) => {
      ...
      if (account && account.unclaimed) {
        ...
        if (account.btc) {
          ...
          var priv = child
            .execSync(
              app.config.bxCommand + " ek-to-ec '" + escpassold + "' " + ek,
              app.config.proc_opts
            )
            .toString()
            .replace('\n', '');
  ...
  accountManager.getTxs = async function (myaccount, token) {
    return new Promise(async (resolve, reject) => {
      var txs = await app.db
        .indexedtx()
        .find({ token: token, $or: [{ from: myaccount }, { to: myaccount }] })
        .sort({ date: -1 })
        .toArray();
      resolve(txs);
    });
  ...
  accountManager.getHolders = async function (token) {
    return new Promise(async (resolve, reject) => {
      var holders = [];
      holders['0xAB8199eba802e7e6634d4389Bf23999b7Ae6b253'] = {
      ...
      for (var i = 0; i < txs.length; i++) {
        ...
        holders[txs[i].from].balance = new BN(holders[txs[i].from].balance)
          .sub(value)
          .toString();
        holders[txs[i].to].balance = new BN(holders[txs[i].to].balance)
          .add(value)
          .toString();
        ...
        await app.db
          ...
          .updateOne(
            { address: txs[i].from },
            { $set: { balance: holders[txs[i].from].balance } },
            { upsert: true }
          );
        await app.db
          .balance2()
          .updateOne(
            { address: txs[i].to },
            { $set: { balance: holders[txs[i].to].balance } },
            { upsert: true }
          );
      }
  ...
  accountManager.getSubscription = async function (myaccount) {
    return new Promise(async (resolve, reject) => {
      var docs = await app.db
        .txs()
        .find({ from: myaccount, to: app.config.atayenSubscriptionAddress })
        .sort({ date: 1 })
        .toArray();
      resolve(docs);
    });
  ...
  accountManager.getBonus = async function (myaccount) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        var h1 = app.web3.utils.sha3(
          app.web3.eth.abi.encodeParameters(['address', 'uint256'], [myaccount, amount])
        );
        ...
        var gas = 100000;
        var receipt = await ctrBonus.methods
          .getGap(myaccount, amount, v, r, s)
          .send({ from: myaccount, gas: gas, gasPrice: gasPrice });
  ...
  accountManager.wrapSatt = async function (amount, cred) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        var gasPrice = await app.web3.eth.getGasPrice();
        var gas = await app.token.contract.methods
          .transfer(addr, amount)
          .estimateGas({ from: cred.address, gasPrice: gasPrice });
        var receipt = await app.token.contract.methods
          .transfer(addr, amount)
          .send({ from: cred.address, gas: gas, gasPrice: gasPrice });
        resolve(receipt.hash);
      } catch (e) {
        reject({ message: e.message });
      }
  ...
  accountManager.unWwrapSatt = async function (amount, cred) {
    return new Promise(async (resolve, reject) => {
      try {
        var gasPrice = await app.web3.eth.getGasPrice();
        var gas = await ctrwSaTT
          .methods()
          .contributeWSATT(amount)
          .estimateGas({ from: cred.address, gasPrice: gasPrice });
        var receipt = await ctrwSaTT
          .methods()
          .contributeWSATT(amount)
          .send({ from: cred.address, gas: gas, gasPrice: gasPrice });
        resolve(receipt.hash);
  ...
  accountManager.create2FA = async function (userId) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        await app.db
          .wallet()
          .updateOne({ UserId: parseInt(userId) }, { $set: { G2FA: secret.base32 } });
    ...
    return new Promise(async (resolve, reject) => {
      try {
            ...
            Total_balance +=
              app.token.filterAmount(
                new Big(balance['amount'] * 1)
                  .div(new Big(10).pow(token_info[T_name].dicimal))
                  .toNumber() + ''
        ...
        for (const Amount in ret) {
          ...
          Total_balance +=
            app.token.filterAmount(
              new Big(ret[Amount] * 1).div(new Big(10).pow(decimal)).toNumber() + ''
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        for (const Amount in ret) {
          ...
          crypto.total_balance =
            app.token.filterAmount(
              new Big(ret[Amount]).div(new Big(10).pow(decimal)).toNumber() + ''
            ) * CryptoPrices[tokenSymbol].price;
      ...
      .find(
        {
          $and: [
            { userSatt: true },
            { hasWallet: true },
            { [queryField]: { $nin: [today] } },
          ],
        },
        { projection: { daily: true, weekly: true, monthly: true } }
      )
      .toArray();
  ...
  accountManager.notificationManager = async (id, NotifType, label) => {
    ...
    let user = await app.db
      .sn_user()
      .findOne({ _id: +id }, { projection: { fireBaseAccessToken: true, _id: false } });
  ...
  accountManager.isBlocked = async (user, auth = false) => {
    ...
    if (auth) {
      if (user.account_locked) {
        if (
          accountManager.differenceBetweenDates(user.date_locked, dateNow) <
      ...
      if (user.account_locked) {
        ...
        accountManager.differenceBetweenDates(user.dateFirstAttempt, dateNow) <
    ...
    if (Object.keys(logBlock).length)
      await app.db.sn_user().updateOne({ _id: user._id }, { $set: logBlock });
    ...
    if (
      ...
      var priceMap = response.data.map((elem) => {
        ...
        obj = {
          ...
          logo:
            'https://s2.coinmarketcap.com/static/img/coins/128x128/' + elem.id + '.png',
        };
        return obj;
      });
      var finalMap = {};
