...
module.exports = async function (app) {
    ...
    oracleManager.linkedin = async (
        ...
        return new Promise(async (resolve, reject) => {
            try {
                var perf = { shares: 0, likes: 0, views: 0 }
                let url = app.config.linkedinStatsUrl(
                    type,
                    idPost,
                    organization
                )
                ...
                if (body.elements.length) {
                    perf.views =
                        body.elements[0]?.totalShareStatistics.impressionCount
                    perf.likes =
                        body.elements[0]?.totalShareStatistics.likeCount
                    perf.shares =
                        body.elements[0]?.totalShareStatistics.shareCount
                }
                if (type !== 'share') {
                    const linkedinVideoData = {
                        ...
                        headers: {
                            Authorization:
                                'Bearer ' + linkedinProfile.accessToken,
                        },
                        json: true,
                    }
