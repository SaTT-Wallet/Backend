...
const {
    ...
    instagram,
    linkedin,
} = require('@atayen-org/statistic-social-network')
...
const facebookStats = async (pageName, idPost) => {
    try {
        ...
        if (page.id) {
            const { token, id } = page
            const result = await facebook(token, id, idPost)
...
const youtubeStats = async (idPost) => {
    try {
...
const instagramStats = async (UserId, link) => {
    try {
        ...
        if (fbPage.token && fbPage.instagram_id) {
...
const twitterStats = async (userName, idPost) => {
    try {
        ...
        if (twitterProfile) {
            const result = await twitter(
                idPost,
                twitterProfile.access_token_key,
                oauth.twitter.consumer_secret,
                oauth.twitter.consumer_key,
                twitterProfile.access_token_secret
            )
