    ...
    try {
        ...
        app.web3 = new Web3(
            new Web3.providers.WebsocketProvider(
                app.config.ethBridge,
                options /*"wss://ropsten.infura.io/ws/v3/0fab761c6a7f4934a892dddebf80f8e0"*/
            )
        )

        app.web3Bep20Websocket = new Web3(
            new Web3.providers.WebsocketProvider(
                app.config.web3UrlBep20Websocket
            )
        )
        ...
        bridge.getCronCtrs = () => {
            var web3eth = new Web3(
                new Web3.providers.WebsocketProvider(
                    app.config.web3Url,
                    options
                )
            )
            var web3Bsc = new Web3(
                new Web3.providers.WebsocketProvider(
                    app.config.web3UrlBep20Websocket,
                    options
                )
        ...
        bridge.initEventHandlers = async () => {
            bridge.contractEvtEth = new app.web3.eth.Contract(
                app.config.ctrs.token.abi,
                app.config.ctrs.token.address.mainnet
            )
            ...
            bridge.contractEvtEth.events.Transfer(
                { filter: { to: app.config.SattBep20Addr } },
                bridge.eventETHtoBSC
            )
        }

        bridge.unlockOwnerBsc = async () => {
            app.web3Bep20Websocket.eth.accounts.wallet.decrypt(
                [app.config.sattBep20],
        ...
        bridge.eventETHtoBSC = async (error, evt) => {
            ...
            var dbl = await app.db
                .bep20()
                .findOne({ ethTxHash: evt.transactionHash })
        ...
        bridge.eventBSCtoETH = async (error, evt) => {
            ...
            var dbl = await app.db
                .bep20()
                .findOne({ bscTxHash: evt.transactionHash })
            if (dbl) {
                return
            }
            ...
            if (
                from == nullAddress ||
                to == nullAddress ||
                to.toLowerCase() != app.config.SattBep20Addr.toLowerCase()
            ) {
                console.log('dropped')
                return
            } else {
        ...
        bridge.bscToEth = async (ctrs, to, value) => {
            ctrs.web3Bsc.eth.accounts.wallet.decrypt(
                [app.config.sattBep20],
                app.config.SattReservePass
            )
            ctrs.web3eth.eth.accounts.wallet.decrypt(
                [app.config.sattBep20],
                app.config.SattReservePass
            )
            ...
            var receiptBurn = await ctrs.ctrBsc.methods.burn(value).send({
                from: app.config.SattBep20Addr,
                gas: gas,
                gasPrice: gasPriceBsc,
            })
            var receiptTransfer = await ctrs.ctrEth.methods
                .transfer(to, value)
                .send({
                    from: app.config.SattBep20Addr,
                    gas: gas,
            ...
            return {
                burnTxHash: receiptBurn.transactionHash,
                ethTxHash: receiptTransfer.transactionHash,
            }
        }
        ...
        bridge.ethToBsc = async (ctrs, to, value) => {
            ...
            var receiptMint = await ctrs.ctrBsc.methods.mint(value).send({
                from: app.config.SattBep20Addr,
                gas: gas,
                gasPrice: gasPriceBsc,
            })
            var receiptTransfer = await ctrs.ctrBsc.methods
                .transfer(to, value)
                .send({
                    from: app.config.SattBep20Addr,
                    gas: gas,
            ...
            return {
                mintTxHash: receiptMint.transactionHash,
                bscTxHash: receiptTransfer.transactionHash,
            }
        }
        ...
        bridge.poll = async () => {
            ...
            for (var i = 0; i < txs.length; i++) {
                ...
                if (tx.type == 'BSC-ETH') {
                    ...
                    await app.db.bep20().updateOne(
                        { _id: tx._id },
                        {
                            $set: {
                                status: 'ok',
                                burnTxHash: hashes.burnTxHash,
                                ethTxHash: hashes.ethTxHash,
                            },
                        }
                    )
                }
