...
module.exports = async function (app) {
    ...
    ContractToken.eventCallback = async function (evt) {
        if (
            evt.signature !=
                '0xb67322f1a9b0ad182e2b242673f8283103dcd6d1c8a19b47ff5524f89d9758ed' ||
            evt.event != 'AskRequest' ||
            evt.event == 'AskRequestBounty'
        ) {
            return
        }
        ...
        var idRequest = isBounty
            ? evt.returnValues.idProm
            : evt.returnValues.idRequest
    ...
    ContractToken.answerOne = async (
        ...
        switch (typeSN) {
            ...
            case '4':
                ...
                break
            case '5':
                var res = await app.oracle.linkedin(
                    idUser,
                    idPost,
                    type,
                    linkedinProfile
    ...
    ContractToken.answerAbos = async (
        typeSN,
        idPost,
        idUser,
        linkedinProfile = null
    ...
    ContractToken.checkAnswer = async function () {
        ...
        for (var i = 0; i < requests.length; i++) {
            ...
            switch (request.typeSN) {
                case '1':
                    var res = await app.oracle.facebook(
                        request.idUser,
                        request.idPost
                    )
                ...
                case '3':
                    ...
                    break
                case '4':
                    var res = await app.oracle.twitter(
                        request.idUser,
                        request.idPost
                    )
            ...
            if (
                ...
                await ContractToken.answerCall({
                    from: app.config.oracleOwner,
                    campaignContract: app.campaign.contract.options.address,
    ...
    ContractToken.checkAnswerBounty = async function () {
        try {
            ...
            for (var i = 0; i < requests.length; i++) {
                ...
                var nbAbos = ContractToken.answerAbos(
                    request.typeSN,
                    request.idPost,
                    request.idUser
                )
                ...
                await ContractToken.answerBounty({
                    from: app.config.oracleOwner,
                    campaignContract: app.campaign.contract.options.address,
                    idProm: request.id,
                    nbAbos: nbAbos,
    ...
    ContractToken.followContract = async function () {
        if (app.config.testnet) {
            ContractToken.contract = new app.web3.eth.Contract(
                app.config.ctrs.oracle.abi,
                app.config.ctrs.oracle.address.testnet
            )
        } else {
            ContractToken.contract = new app.web3.eth.Contract(
                app.config.ctrs.oracle.abi,
                app.config.ctrs.oracle.address.mainnet
            )
        }
        ...
        if (app.config.testnet) {
            ContractToken.contractBep20 = new app.web3Bep20.eth.Contract(
                app.config.ctrs.oracle.abi,
                app.config.ctrs.oracle.address.testnetBep20
            )
            ContractToken.contractBep20WS =
            ...
            ContractToken.contractBep20 = new app.web3Bep20.eth.Contract(
                app.config.ctrs.oracle.abi,
                app.config.ctrs.oracle.address.mainnetBep20
            )
            ContractToken.contractBep20WS =
                new app.web3Bep20Websocket.eth.Contract(
                    app.config.ctrs.oracle.abi,
        ...
        ContractToken.contractBep20WS.events.allEvents(async function (
            err,
            evt
        ) {
    ...
    ContractToken.addCampaign = async function (addr) {
        return new Promise(async (resolve, reject) => {
            ...
            var gas = await ContractToken.contract.methods
                .changeAsk(addr, true)
                .estimateGas()
            var gasPrice = await app.web3.eth.getGasPrice()
            ContractToken.contract.methods
                .changeAsk(addr, true)
                .send({
                    from: app.config.oracleOwner,
                    gas: gas,
                ...
                .on('error', function (error) {
                    if (!headerSent) reject({ error: '' + error })
