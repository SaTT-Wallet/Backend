module.exports = async function (app) {
    ...
    cryptoManager.transfer = async function (to, amount, credentials) {
        return new Promise(async (resolve, reject) => {
            ...
            try {
                ...
                var receipt = await app.web3.eth
                    .sendTransaction({
    ...
    cryptoManager.sendBtc = async function (id, pass, to, amount) {
        ...
        if (parseInt(amount) + parseInt(fee) > max) {
            return {
                error:
                    'insufficient funds, fee requirement : ' +
                    fee +
                    ' satoshis',
            }
        }
    ...
    cryptoManager.getReceiveBtcWallet = async function (userId) {
        return new Promise(async (resolve, reject) => {
            ...
            if (!account) reject(false)
                ...
                await rp({
                    uri: app.config.btcElectrumUrl + 'pubkey/',
                    method: 'POST',
                    body: { pubkey: pub },
                    json: true,
                })
                ...
                await Wallet.updateOne(
                    { UserId: parseInt(userId) },
                    {
                        $set: {
                            BtcHDPublicKey: pub,
                            BtcHD: address1,
                            BtcHDSegWitCompat: address3,
                            BtcHDSegWit: addressbc1,
                        },
                    }
                )
                resolve(address3)
    ...
    cryptoManager.listBtcPayers = async function () {
        return new Promise(async (resolve, reject) => {
            ...
            var utxos = JSON.parse(
                child.execSync(app.config.btcCmd + ' listunspent 0 1000000')
            )
            ...
            else amounts[amounts.address] = parseFloat(amounts.amount)

            for (var i = 0; i < results.length; i++) {
                if (
                    amounts[results[i].BtcHDSegWitCompat] &&
                    amounts[results[i].BtcHDSegWitCompat] > 0.00000547
                ) {
                    listbtc.push({
                        id: results[i].UserId,
                        BtcHDSegWitCompat: results[i].BtcHDSegWitCompat,
                        balance: amounts[results[i].BtcHDSegWitCompat],
                    })
                }
                //touched
            }
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (bnbal.lt(bngas.mul(bngasPrice))) {
                ...
                var signed = await app.web3Inf.eth.accounts.signTransaction(
                    tx,
                    '0x' + myWallet.getPrivateKey().toString('hex')
                )
                var receipt = await app.web3Inf.eth
                    .sendSignedTransaction(signed.rawTransaction)
                    .once('transactionHash', async function (hash) {})

                var p = await app.db.rate().findOne({
                    symbol: 'ETH',
                    date: { $gt: Date.now() - 600000 },
                })

                var bnamount = bnbal.mul(
                    new BN(
                        Math.floor(
                            (parseFloat(p.price) / 0.0042) *
                                app.config.icoFactor
                        )
                    )
                )
                ...
                var res = await app.db.sattbuy().insertOne({
                    UserId: userId,
                    to: to,
                ...
                resolve({
                    result: 'OK',
                    hash: res.insertedId,
                    amount: tx.value / app.config.EtherWei,
                })
            }
        })
    }
    ...
    cryptoManager.receiveBtcWallet = async function (userId) {
        return new Promise(async (resolve, reject) => {
            ...
            // btc electrum
            var utxo = await rp({
                uri: app.config.btcElectrumUrl + 'utxo/' + address3,
                json: true,
            })
            if (!utxo.length) {
            ...
            if (amountfee <= 0) {
                ...
                // btc electrum
                var rec = await rp({
                    uri: app.config.btcElectrumUrl + 'tx/',
                    method: 'POST',
                    body: { tx: signed },
                    json: true,
                })
                ...
                var p = await app.db.rate().findOne({
                    symbol: 'BTC',
                    date: { $gt: Date.now() - 800000 },
                })

                var bn10 = new BN('10000000000')
                var bn = new BN(
                    Math.floor(
                        ((amount * parseFloat(p.price)) / 0.42) *
                            app.config.icoFactor *
                            100
                    )
                )
    ...
    cryptoManager.FilterTransactionsByHash = (
        All_Transactions,
        Erc20_OR_BEP20_Transactions,
        Network
    ) => {
        var transaction_content = All_Transactions.result
        var erc20_or_bep20_transaction_content =
            Erc20_OR_BEP20_Transactions.result

        transaction_content.map((elem) => {
            for (
                var i = 0;
                i < erc20_or_bep20_transaction_content.length;
                i++
            ) {
