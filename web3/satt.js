module.exports = async function (app) {
    ...
    tokenManager.followContract = function () {
        if (app.config.testnet) {
            tokenManager.contract = new app.web3.eth.Contract(
    ...
    tokenManager.approve = async function (addr, spender, amount) {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                var receipt = await tokenManager.contract.methods
    ...
    tokenManager.getApproval = async function (addr, spender) {
        return new Promise(async (resolve, reject) => {
            var amount = await tokenManager.contract.methods
                .allowance(addr, spender)
                .call()
    ...
    tokenManager.transfer = async function (to, amount, credentials) {
        return new Promise(async (resolve, reject) => {
            ...
            if (credentials.gasPrice) gasPrice = credentials.gasPrice
    ...
    tokenManager.estimateApprove = async function (addr, spender, amount) {
        return new Promise(async (resolve, reject) => {
            try {
                var gasPrice = await app.web3.eth.getGasPrice()
                var gas = await tokenManager.contract.methods
                    .approve(spender, amount)
                    .estimateGas({ from: addr, gasPrice: gasPrice })
                resolve(gas)
            } catch (err) {
                reject(err)
            }
